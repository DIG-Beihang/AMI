...
def process_env(env, pipe):
    while True:
        ...
        elif command[0] == "get_obs":
            ret = env.get_obs()
        elif command[0] == "get_obs_size":
            ret = env.get_obs_size()
        elif command[0] == "get_state":
            ret = env.get_state()
        elif command[0] == "get_state_size":
            ret = env.get_state_size()
        elif command[0] == "get_avail_actions":
            ret = env.get_avail_actions()
...
class StarCraft2TwoPlayerEnv(MultiAgentEnv):
    def __init__(self, **kwargs):
        self.host_env = StarCraft2TwoPlayerEnvKernel(**kwargs, host=True)
        self.client_env = StarCraft2TwoPlayerEnvKernel(**kwargs, host=False)
        ctx = mp.get_context()
        self.host_pipe, self.host_child_pipe = mp.Pipe()
        self.client_pipe, self.client_child_pipe = mp.Pipe()
        self.p_host_env = ctx.Process(target=process_env, args=(self.host_env, self.host_child_pipe))
        self.p_client_env = ctx.Process(target=process_env, args=(self.client_env, self.client_child_pipe))
    ...
    def step(self, actions1, actions2):
        ...
        self.host_pipe.send(["step", actions1])
        self.client_pipe.send(["step", actions2])
        return [self.host_pipe.recv(), self.client_pipe.recv()]

    def get_obs(self):
        """Returns all agent observations in a list."""
        self.host_pipe.send(["get_obs"])
        self.client_pipe.send(["get_obs"])
        return [self.host_pipe.recv(), self.client_pipe.recv()]

    def get_obs_size(self):
        """Returns the size of the observation."""
        self.host_pipe.send(["get_obs_size"])
        self.client_pipe.send(["get_obs_size"])
        return [self.host_pipe.recv(), self.client_pipe.recv()]

    def get_state(self):
        """Returns the global state."""
        self.host_pipe.send(["get_state"])
        self.client_pipe.send(["get_state"])
        return [self.host_pipe.recv(), self.client_pipe.recv()]

    def get_state_size(self):
        """Returns the size of the global state."""
        self.host_pipe.send(["get_state_size"])
        self.client_pipe.send(["get_state_size"])
        return [self.host_pipe.recv(), self.client_pipe.recv()]

    def get_avail_actions(self):
        """Returns the available actions of all agents in a list."""
        self.host_pipe.send(["get_avail_actions"])
        self.client_pipe.send(["get_avail_actions"])
        return [self.host_pipe.recv(), self.client_pipe.recv()]
...
class StarCraft2TwoPlayerEnvKernel(MultiAgentEnv):

...
class ACTLayer(nn.Module):
    ...
    def get_prob(self, x, available_actions=None):
        """
        Compute action probabilities from inputs.
        :param x: (torch.Tensor) input to network.
        :param available_actions: (torch.Tensor) denotes which actions are available to agent
        ...
        if self.mixed_action or self.multi_discrete:
            action_probs = []
            for action_out in self.action_outs:
                action_prob = action_out.transit(x)
                action_probs.append(action_prob)
            action_probs = torch.cat(action_probs, -1)
        else:
            action_probs = self.action_out.transit(x, available_actions)
    ...
    def evaluate_actions(self, x, action, available_actions=None, active_masks=None):
        ...
        if self.mixed_action:
            ...
            if active_masks is not None:
                if self.action_type=="Discrete":
                    dist_entropy = (action_logits.entropy()*active_masks.squeeze()).sum()/active_masks.sum()
                else:
                    dist_entropy = (action_logits.entropy()*active_masks).mean(dim=-1).sum()/active_masks.sum()
            else:

...
class MujocoMulti(MultiAgentEnv):
    ...
    def get_obs(self):
        ...
        for a in range(self.n_agents):
            ...
            # obs_n.append(obs_i)
            obs_lst = np.concatenate([agent_id_feats, self.get_obs_agent(a)])
            if getattr(self, "obs_size", None) is not None:
                pad = np.array((self.obs_size - len(obs_lst))*[0])
                if len(pad):
                    obs_lst = np.concatenate([np.array(obs_lst), pad])
            obs_n.append(obs_lst)
        return obs_n
    ...
    def get_obs_size(self):
        ...
        if self.agent_obsk is None:
            ...
            return max([len(obs) for obs in self.get_obs()])
            # return max([len(self.get_obs_agent(agent_id)) for agent_id in range(self.n_agents)])

    def get_state(self, team=None):
        """ The original version of Multi-Agent MuJoCo"""
    ...
    # def get_state(self, team=None):
    #     """ The modified version by HATRPO"""
    #     # TODO: May want global states for different teams (so cannot see what the other team is communicating e.g.)
    #     state = self.env._get_obs()
    #     share_obs = []
    #     for a in range(self.n_agents):

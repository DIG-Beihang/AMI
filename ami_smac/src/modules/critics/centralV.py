...
class CentralVCritic(nn.Module):
    ...
    def _build_inputs(self, batch, t=None):
        ...
        if self.args.mode == "adv_policy":
            agent_index = th.eye(self.args.n_agents, device=batch.device).unsqueeze(0).unsqueeze(0).expand(bs, max_t, -1, -1)
            adv_agent_mask = batch["adv_agent_mask"][:, 0:1].expand_as(agent_index)
            inputs.append(agent_index[adv_agent_mask].reshape(bs, max_t, self.n_agents, -1))
        else:

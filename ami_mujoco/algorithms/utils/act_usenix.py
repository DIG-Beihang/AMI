from .distributions import Bernoulli, FixedCategorical, init, FixedNormal
import torch
import torch.nn as nn

class CategoricalUSENIX(nn.Module):
    def __init__(self, args, num_inputs, num_outputs, use_orthogonal=True, gain=0.01):
        super(CategoricalUSENIX, self).__init__()
...
class DiagGaussianUSENIX(nn.Module):
    def __init__(self, args, num_inputs, num_outputs, use_orthogonal=True, gain=0.01):
        super(DiagGaussianUSENIX, self).__init__()
...
class ACTLayer(nn.Module):
    ...
    def __init__(self, action_space, inputs_dim, use_orthogonal, gain, args=None):
        ...
        if action_space.__class__.__name__ == "Discrete":
            ...
            self.action_out = CategoricalUSENIX(args, inputs_dim, action_dim, use_orthogonal, gain)
        elif action_space.__class__.__name__ == "Box":
            action_dim = action_space.shape[0] * args.num_agents
            self.action_out = DiagGaussianUSENIX(args, inputs_dim, action_dim, use_orthogonal, gain)
            ...
            for action_dim in action_dims:
                self.action_outs.append(CategoricalUSENIX(inputs_dim, action_dim, use_orthogonal, gain))
            self.action_outs = nn.ModuleList(self.action_outs)
        else:  # discrete + continous
            self.mixed_action = True
            continous_dim = action_space[0].shape[0]
            discrete_dim = action_space[1].n
            self.action_outs = nn.ModuleList([DiagGaussianUSENIX(inputs_dim, continous_dim, use_orthogonal, gain, args),
                                              CategoricalUSENIX(inputs_dim, discrete_dim, use_orthogonal, gain)])
